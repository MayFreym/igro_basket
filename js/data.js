...
window.Cart.data = (() => {
  ...
  //
  // Описывают форму данных, но в вид не попадают: name, art, img. Имена,
  // артикулы и картинки в разметке статические — view.js их не рисует вообще.
  // calc.js всё равно готовит name/meta/imgUrl на каждый пересчёт и выбрасывает.
  // Выкидывать не спешить: в бою эти поля напечатает $arResult. См. FIXES.md #10.
  //   brand — исключение, живой: по нему считается повышенная скидка.
  const catalog = [
    {
  ...
  //   badges     — бейджи в плашке статуса; t — текст, tip — текст подсказки
  //
  // Ничем не читается:
  //   noBand     — про запас
  //   priceRoot  — мёртв, хотя выглядит живым. Единственный читатель —
  //                `cur.noDiscLabel || priceRoot` в calc.js: ветка срабатывает
  //                только при нулевой скидке, то есть у rrc и master, а у обоих
  //                noDiscLabel задан. Остальные статусы имеют base >= 10 и до
  //                неё не доходят. См. FIXES.md #10.
  //   badges.short — ничем не читается; renderStatusBand берёт только t и tip
  const statuses = [
